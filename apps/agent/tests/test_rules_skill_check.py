...
class TestResolveSkillCheck:
    def test_success(self):
        ...
        rng = random.Random(42)
        result = resolve_skill_check(SAMPLE_PLAYER, "athletics", "moderate", rng=rng, ally_present=False)
    ...
    def test_nat_20_always_succeeds(self):
        ...
        for seed in range(1000):
            ...
            if rng.randint(1, 20) == 20:
                rng = random.Random(seed)
                result = resolve_skill_check(SAMPLE_PLAYER, "persuasion", "very_hard", rng=rng, ally_present=False)
    ...
    def test_nat_1_always_fails(self):
        for seed in range(1000):
            ...
            if rng.randint(1, 20) == 1:
                rng = random.Random(seed)
                result = resolve_skill_check(SAMPLE_PLAYER, "athletics", "easy", rng=rng, ally_present=False)
    ...
    def test_proficiency_bonus_applied(self):
        rng = random.Random(42)
        prof_result = resolve_skill_check(SAMPLE_PLAYER, "athletics", "moderate", rng=rng, ally_present=False)
        rng = random.Random(42)
        unprof_result = resolve_skill_check(SAMPLE_PLAYER, "persuasion", "moderate", rng=rng, ally_present=False)
...
class TestResolveSkillCheckDc:
    def test_success_with_numeric_dc(self):
        rng = random.Random(42)
        result = resolve_skill_check_dc(SAMPLE_PLAYER, "athletics", 10, rng=rng, ally_present=False)
        assert result.dc == 10
        assert result.skill == "athletics"
        assert result.modifier == 5  # STR +2, trained: prof +1 + tier +2
    ...
    def test_failure_with_high_dc(self):
        for seed in range(1000):
            ...
            if d20 != 20 and d20 + 4 < 25:
                rng = random.Random(seed)
                result = resolve_skill_check_dc(SAMPLE_PLAYER, "athletics", 25, rng=rng, ally_present=False)
                assert result.success is False
                assert result.dc == 25
                return
    ...
    def test_nat_20_always_succeeds(self):
        ...
        for seed in range(1000):
            ...
            if rng.randint(1, 20) == 20:
                rng = random.Random(seed)
                result = resolve_skill_check_dc(SAMPLE_PLAYER, "perception", 23, rng=rng, ally_present=False)
                assert result.success is True
                assert result.roll == 20
                return
        pytest.fail("Could not find seed for nat 20")
    ...
    def test_nat_1_always_fails(self):
        for seed in range(1000):
            ...
            if rng.randint(1, 20) == 1:
                rng = random.Random(seed)
                result = resolve_skill_check_dc(SAMPLE_PLAYER, "athletics", 1, rng=rng, ally_present=False)
                assert result.success is False
                assert result.roll == 1
                return
        pytest.fail("Could not find seed for nat 1")
    ...
    def test_uses_numeric_dc_not_tier(self):
        rng = random.Random(42)
        dc_result = resolve_skill_check_dc(SAMPLE_PLAYER, "athletics", 14, rng=rng, ally_present=False)
        assert dc_result.dc == 14
        rng = random.Random(42)
        tier_result = resolve_skill_check(SAMPLE_PLAYER, "athletics", "moderate", rng=rng, ally_present=False)
        assert tier_result.dc == 12  # moderate = 12, not 14
        assert dc_result.dc != tier_result.dc

    def test_proficiency_applied(self):
        rng = random.Random(42)
        result = resolve_skill_check_dc(SAMPLE_PLAYER, "perception", 10, rng=rng, ally_present=False)
