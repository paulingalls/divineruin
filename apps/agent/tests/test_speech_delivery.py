...
def _reply_handles(tree: ast.AST) -> set[str]:
    """Names bound to a ``generate_reply`` call — the handle a delivery would read."""
    handles: set[str] = set()
...
def _delivery_owners(sources: dict[str, str]) -> tuple[set[str], set[str]]:
    ...
    for name, source in sources.items():
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Await) and isinstance(node.value, ast.Name) and node.value.id in handles:
                awaiters.add(name)
