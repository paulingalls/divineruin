...
import pytest
from livekit.agents import StopResponse, llm, room_io
...
async def test_two_existing_participants_get_exact_room_options_and_bound_transcripts() -> None:
    ...
    assert await manager.receive() == AuthenticatedTranscript("player-two", "second phrase")
    assert await manager.receive() == AuthenticatedTranscript("player-one", "first phrase")
...
async def test_unknown_identity_is_preserved_and_empty_identity_fails() -> None:
    ...
    assert (await manager.receive()).participant_identity == "unregistered-player"
    with pytest.raises(ValueError, match="identity is empty"):
        room.emit("participant_connected", Participant(""))
    await manager.aclose()


async def test_empty_completed_transcript_fails_before_emission() -> None:
    room = Room(("player-one",))
    factory = Factory()
    manager = MultiParticipantTranscriber(room, stt=None, session_factory=factory)
    manager.start()
...
async def test_listener_precedes_inventory_and_snapshot_join_is_deduplicated() -> None:
    room = Room(("player-one",))
    original = room.remote_participants
    ...
    room.remote_participants = Inventory(original)
    factory = Factory()
    manager = MultiParticipantTranscriber(room, stt=None, session_factory=factory)
    manager.start()
    await settle()
...
async def test_start_and_session_errors_reach_consumer_with_identity_and_cause() -> None:
    ...
    manager.start()
    await settle()
    factory.calls[0][3].emit_error(ConnectionError("stream broke"))
    with pytest.raises(RuntimeError, match=r"bad-stream.*stream broke") as caught:
        await manager.receive()
