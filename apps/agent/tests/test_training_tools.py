"""Tests for training-cycle agent tools (M1.5)."""
...
from training_tools import (
    _initiate_training_cycle_impl,
    _query_training_programs_impl,
    _resolve_training_midpoint_impl,
)
...
def _make_context(player_id: str = "player_1") -> MagicMock:
    ctx = MagicMock()
...
class TestInitiateTrainingCycle:
    ...
    async def test_happy_path_creates_row(self):
        ctx = _make_context()
        mock_db, mock_conn = _make_db_mod()
        mock_content = MagicMock()
...
class TestResolveTrainingMidpoint:
    ...
    async def test_not_owned_returns_error(self):
        ctx = _make_context(player_id="player_2")
        mock_db, _ = _make_db_mod()
        mock_training = MagicMock()
        mock_training.get_training_activity = AsyncMock(return_value=SAMPLE_AWAITING_ROW)
        mock_training.update_training_activity = AsyncMock()
...
# --- CityAgent registration (story-004) ---
