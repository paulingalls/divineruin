...
class TestEventBus:
    ...
    async def test_drain_returns_all_queued(self):
        bus = EventBus()
        for i in range(5):
            await bus.publish(GameEvent(event_type=f"event_{i}", payload={"i": i}))
        assert bus.qsize == 5
        events = bus.drain()
        assert len(events) == 5
        assert bus.qsize == 0
    ...
    async def test_overflow_drops_oldest(self):
        ...
        await bus.publish(GameEvent(event_type="first", payload={}))
        await bus.publish(GameEvent(event_type="second", payload={}))
        await bus.publish(GameEvent(event_type="third", payload={}))
        # Queue is full — next publish drops oldest
        await bus.publish(GameEvent(event_type="fourth", payload={}))
        assert bus.qsize == 3
        events = bus.drain()
