...
class _AccessorCallVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Attribute) and node.func.attr == "get_single_skill_advancement":
            self.calls.append((self.path.as_posix(), self.functions[-1] if self.functions else None))
        self.generic_visit(node)


def test_only_persistence_reads_single_skill_advancement():
    ...
    assert calls == [("skill_persistence.py", "apply_skill_use_with_persistence")]
