...
async def test_begin_activity_companion_errand_creates_an_async_activity_row(reset_db_pool: str) -> None:
    ...
    try:
        # Kael complements the mage line; dispatch gates on the ASSIGNED companion (sprint-046),
        # so the seeded class must be one Kael is assigned to.
        await seed_player(pool, player_id=player_id, class_="mage", companion={"id": "companion_kael", "name": "Kael"})
