Feature: M1.5 training cycle — DM training agent end-to-end
  ...
  Postgres testcontainer via the LiveKit test framework. Training is reached on
  DispatchAgent through the begin_activity/resolve_activity verbs (M26 fold), when
  the player enters the training hall.
  ...
  Scenario: Player resolves the midpoint decision
    ...
    Then the agent calls the "resolve_activity" tool
