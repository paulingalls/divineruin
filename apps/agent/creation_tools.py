...
VALID_MUSIC_STATES = {"wonder", "sorrow", "tension", "silence"}
...
async def push_creation_music(music_state: str, room, event_bus) -> None:
    ...
    if music_state not in VALID_MUSIC_STATES:
        raise ValueError(f"Invalid music state: {music_state}. Valid: {', '.join(sorted(VALID_MUSIC_STATES))}")
...
async def finalize_character(context: RunContext) -> str | tuple:
    ...
    # Tool-return handoff: return (OnboardingAgent, json_str) tuple.
