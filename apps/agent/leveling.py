...
MAX_SPELL_TIER_BY_LEVEL: dict[str, int] = {
    "cantrip": 1,
    "minor": 1,
    "standard": 4,
    "major": 7,
...
def is_spell_tier_unlocked(tier: str, level: int) -> bool:
    ...
    """
    if tier not in MAX_SPELL_TIER_BY_LEVEL:
        raise ValueError(f"unknown spell tier {tier!r}; expected one of {sorted(MAX_SPELL_TIER_BY_LEVEL)}")
    return level >= MAX_SPELL_TIER_BY_LEVEL[tier]
