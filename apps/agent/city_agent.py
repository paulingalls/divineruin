...
logger = logging.getLogger("divineruin.city")
...
class CityAgent(BaseGameAgent):
    """Gameplay agent for settlement/city exploration.

    Owns BackgroundProcess for warm layer rebuilds, companion speech, and
    proactive events. Uses build_system_prompt() which assembles the city
    prompt (no combat rules, no creation instructions).
    """
    ...
    async def on_enter(self) -> None:
        ...
        logger.info("CityAgent entered session")
    ...
    async def on_exit(self) -> None:
        logger.info("CityAgent exiting session")
        sd: SessionData = self.session.userdata
