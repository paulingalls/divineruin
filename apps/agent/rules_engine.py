...
from archetypes import PoolFormula, ResourcePattern, get_archetype_chassis
...
# Per-archetype HP/resource chassis now lives in content/archetypes.json (loaded
# via archetypes.get_archetype_chassis). This module owns only the pool math.
...
def calculate_max_pools(
    ...
    """
    resource = get_archetype_chassis(archetype).resource
    pattern, stamina_formula, focus_formula = (
        resource.pattern,
        resource.stamina_formula,
        resource.focus_formula,
    )
