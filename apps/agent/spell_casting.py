...
async def _resolve_cast(
    ...
    *,
    ...
    # Focus gate FIRST — reject before any write so an unaffordable cast deducts nothing (AC1
    # out-of-combat; in combat the phase pre-validates before the loop so nothing else is clobbered).
    # Cantrips (focus_cost 0) skip the gate and the deduction.
