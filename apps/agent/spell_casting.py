...
Focus and deducts it, generates Resonance via the M3.1 rules engine
(resonance.calculate_resonance_generated), accrues that onto the session's
ResonanceTrack and persists it, then returns an effect + narration_cue + audio_cue
packet for the DM to voice. Cantrips (focus_cost 0) cost no Focus, generate 0
Resonance, and scale their damage via leveling.cantrip_damage_dice(level).
...
non-cantrips (cantrips short-circuit to 0 Resonance, and arcane/divine ignore
terrain). No runtime location->terrain map exists yet, so terrain defaults to
"normal"; a primal non-cantrip therefore fails loud as a ToolError until terrain
wiring lands (M3.4). Arcane/divine casting and every cantrip are unaffected.
"""
...
async def _cast_spell_impl(
    ...
    *,
    ...
    async with db_mod.transaction() as conn:
        ...
        # Resonance generated by this cast (rules engine). Compute before deducting so
        # a primal-without-terrain failure still leaves Focus untouched.
        try:
