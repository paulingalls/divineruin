...
// feature sections (Session, World, Races, Pantheon, Classes, Tech). (Pricing + Waitlist
// land in Milestone 5.) Kept hydration-safe — no window/Date/random during render —
// because the same component is rendered on the server (build-time prerender) and hydrated
// on the client (client.tsx); identical markup on both sides prevents a React hydration
// mismatch. Each section imports its own co-located CSS, which Bun pulls into the bundle
// through this import graph.
export function App() {
  return (
    <>
      <NavBar />
