...
test("the root test aggregate fails on a design-token failure", async () => {
  ...
  try {
    ...
    await writeFile(
      join(root, "packages/design-tokens", "fault.test.ts"),
      'import { expect, test } from "bun:test"; test("fault", () => expect(true).toBe(false));\n',
    );

    const child = Bun.spawn(["bun", "run", "test"], { cwd: root, stdout: "pipe", stderr: "pipe" });
    const [stderr, exitCode] = await Promise.all([new Response(child.stderr).text(), child.exited]);
    expect(exitCode).not.toBe(0);
    expect(stderr).toContain("fault.test.ts");
  } finally {
