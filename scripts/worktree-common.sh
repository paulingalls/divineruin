...
import sys
try:
    value, port = sys.argv[1:]
    parsed = urlsplit(value)
    userinfo = parsed.netloc.rpartition("@")[0]
    authority = f"{userinfo}@" if userinfo else ""
    print(urlunsplit((parsed.scheme, f"{authority}127.0.0.1:{port}", parsed.path, parsed.query, parsed.fragment)))
except ValueError:
    print("")
PYURL
}
...
wt_validate_runtime_values() {
  ...
  if [ -n "$db_url" ]; then
    ...
    if [ "$endpoint" != $'postgresql\t127.0.0.1\t'"$POSTGRES_HOST_PORT" ]; then
      wt_die "runtime DATABASE_URL=$db_url conflicts with checkout $WT_CHECKOUT_ID; replace with DATABASE_URL=$(wt_replacement_url "$db_url" "$POSTGRES_HOST_PORT"). Preserve the data, correct the caller environment, then retry."
      return 1
    fi
  fi
  if [ -n "$redis_url" ]; then
    ...
    if [ "$endpoint" != $'redis\t127.0.0.1\t'"$VALKEY_HOST_PORT" ]; then
      wt_die "runtime REDIS_URL=$redis_url conflicts with checkout $WT_CHECKOUT_ID; replace with REDIS_URL=$(wt_replacement_url "$redis_url" "$VALKEY_HOST_PORT"). Preserve the data, correct the caller environment, then retry."
      return 1
    fi
  fi
}
...
wt_validate_settings() {
  ...
  if [ "$db_endpoint" != $'postgresql\t127.0.0.1\t'"$POSTGRES_HOST_PORT" ]; then
    wt_die "DATABASE_URL conflicts with checkout $WT_CHECKOUT_ID and its owned Postgres endpoint 127.0.0.1:$POSTGRES_HOST_PORT; replace with DATABASE_URL=$(wt_replacement_url "$db_url" "$POSTGRES_HOST_PORT"). Preserve the data, correct $file, then retry."
    return 1
  fi
  if [ "$redis_endpoint" != $'redis\t127.0.0.1\t'"$VALKEY_HOST_PORT" ]; then
    wt_die "REDIS_URL conflicts with checkout $WT_CHECKOUT_ID and its owned Valkey endpoint 127.0.0.1:$VALKEY_HOST_PORT; replace with REDIS_URL=$(wt_replacement_url "$redis_url" "$VALKEY_HOST_PORT"). Preserve the data, correct $file, then retry."
    return 1
  fi
