...
async def validate(conn: asyncpg.Connection) -> list[str]:
    ...
    for row in rows:
        ...
        for direction, exit_info in exits.items():
            ...
            if dest not in location_ids:
                errors.append(
                    f"Location '{row['id']}' exit '{direction}' references unknown destination '{dest}'"
                )
    ...
    for row in quest_rows:
        ...
        for stage in data.get("stages", []):
            ...
            for effect in on_complete.get("world_effects", []):
                ...
                if m:
                    shorthand = m.group(1)
                    if not is_valid_disposition_target(
                        shorthand, npc_ids, companion_ids
                    ):
                        errors.append(
    ...
    for row in node_rows:
        ...
        if loc_ref not in location_ids:
            errors.append(
                f"Gathering node '{row['id']}' references unknown location_id '{loc_ref}'"
            )
        res_ref = data.get("resource_type")
        if res_ref not in material_ids:
            errors.append(
                f"Gathering node '{row['id']}' references unknown resource_type '{res_ref}'"
            )
