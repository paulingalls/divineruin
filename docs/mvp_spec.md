...
|---|---|---|
| **Voice input → DM narration** | Core loop | `ExplorationAgent` in LiveKit `AgentSession` with Deepgram STT → GPT-6 Luna → Inworld TTS pipeline. See *Tech Architecture — DM Agent Architecture*. |
...
3. **DM Agent — basic voice loop.** The original `DungeonMasterAgent` proved the voice loop; `ExplorationAgent` now handles gameplay conversation on GPT-6 Luna.
