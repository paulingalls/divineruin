...
- [x] Beat 3 narration pauses for dramatic dice reveals. **Reaction windows RESTORED to Beat 3** (M29 story-016, superseding the M2.2 story-002 reversal): design decision 46 (`game_mechanics_decisions.md:61`) and `game_mechanics_combat.md:130/182/187` are true of the code again. The phase is TWO commits — Beat 2 resolves the ally band and commits, Beat 3 holds each enemy action and pauses for reaction windows, and the wrap rides the last commit because an enemy blow can end the fight. TWO windows per enemy attack: a PRE-ROLL window (`on_targeted` / `on_ally_targeted` / `on_enemy_action`) and a POST-ROLL, PRE-DAMAGE window (`on_hit` / `on_ally_hit` / `on_enemy_miss` / `on_enemy_action`, plus `on_condition_imposed` on a landed grapple). `reaction_windows.py` derives them from the held action; `combat_hold.py` owns the queue and the pause gate. `game_mechanics_combat.md:131` is honoured too — no window opens when no standing player has a reaction available, so the pause never becomes noise. The open window reaches the DM through `resolve_phase`'s `next` field (ADR 0008 decision 4), never guessed. **Activation is wired** (M29 story-017): `activate` is the verb that spends a reaction, gated on the OPEN window rather than on a beat or a pre-declaration — the DM calls it with an id from `next.waiting_on.reactions`, which lists only the reactions `validate_reaction_activation` would accept at that window (free patch 2026-09-13, after a real DM guessed `uncanny_dodge`). The spend records the ability id, the window id, the stage and the held action's `seq` (`reaction_spend.py`), which is the binding story-018 reads to give a reaction mechanical effect. `activate` stays OUT of `next.verbs` deliberately: `verbs` is the ADVANCE set and only `resolve_phase` advances a paused beat — the producer for activation is `next.waiting_on.reactions`, not a verb list.
