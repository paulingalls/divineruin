...
- **Terrain → Phase 3 Magic.** M10.2 edits Resonance calculation, the Veil Ward activation path, and the Korath reduction in `spell_casting.py:187-195`. The retired `activate_veil_ward` DM tool now uses `activate`. `03_magic.md` M3.1 already declares `calculate_resonance_generated(focus_cost, source, terrain=None)` — this phase enumerates the terrain table M3.1 referenced but never listed, and un-deads the routing. Add pointers from `03_magic.md` M3.1 / M3.2 / M3.4 to this milestone so the magic doc isn't silently incomplete. The current DM verb is `activate`.
...
- **Druid Veil Ward** — in `_activate_veil_ward_impl` (`veil_ward_tools.py:61`; the retired `activate_veil_ward` DM tool uses `activate`; `druid` source constant at `veil_ward.py:48`), gate the `druid` source on `is_natural(terrain)`. The "stronger in old-growth / sacred groves" bonus applies only to `ancient_forest`. **→ M24 pointer:** the full-spec ward is now scope-owned (`encounter` keyed by `combat_id`, `location` keyed by `location_id`), party-wide and duration-bound — model in `docs/game_mechanics/veil_ward_scope_model.md`. M24 ships the `druid` source **ungated**; this phase adds the gate to the reworked activation path, which raises a `location` ward out of combat and an `encounter` ward in it. `content/locations.json` has `terrain` and `region_type` but **no `is_natural`** — Phase 10 adds it. Sequence after M24. The current DM verb is `activate`.
