...
- Level-up rewards emission satisfied via `award_xp` agent tool (`apps/agent/progression_tools.py:26`) — `check_level_up` + `build_level_up_payload` + `LEVEL_UP` event publish. No separate `apply_level_up` tool; second entrypoint would introduce drift risk. <!-- M28 story-003: the `award_xp` TOOL was subsequently deleted; the same logic now runs as the `_award_xp_core` Resolve on combat exit and quest completion. The one-entrypoint reasoning above held and is what the removal preserved. -->
