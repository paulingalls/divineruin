...
- Former agent tool: `resolve_companion_errand(errand_id)` → generates return narration Current DM verb: `resolve_activity`. <!-- see audit/phase-1-async.md#m1.6 — resolve_companion_errand exists as a pure function in async_rules.py:133 but is NOT exposed as a @function_tool; invoked only by the async worker -->
