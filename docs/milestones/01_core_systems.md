...
**Acceptance criteria:**
- [x] Each archetype correctly assigned to Stamina-only, Focus-only, Focus-primary, or Split <!-- evidence: apps/agent/archetypes.py DB-loaded archetype chassis (18 entries, 4 patterns); all 18 verified — see audit/phase-1-characters.md §M1.3 Archetype Patterns -->
...
- [x] Unified progression payload available across all 20 levels with attribute points, milestones, proficiency, fork, and per-level HP gain — HP joined from `ARCHETYPE_HP_CONFIG` at lookup time via `build_level_up_payload_for_archetype` <!-- evidence: apps/agent/leveling.py:331 build_level_up_payload_for_archetype joins LEVEL_PROGRESSION + ARCHETYPE_HP_CONFIG; tests apps/agent/tests/test_leveling.py TestArchetypePayload. Production LEVEL_UP publish sites (progression_tools.py:87, quest_tools.py:189) emit the archetype-aware payload via build_level_up_payload_for_archetype (debt a6974311f047 resolved). -->
- [x] Level-up triggers a narration event that the DM agent can consume <!-- evidence: apps/agent/event_types.py:30 LEVEL_UP; apps/agent/progression_tools.py:88 publish; :325 get_milestone_narration. Note: no standalone apply_level_up agent tool — emitted by award_xp/complete_quest paths (deliverable struck by sprint-008). M28 story-003 replaced the award_xp tool with the _award_xp_core Resolve; the publish site moved with it. -->
...
- Former agent tool: `initiate_training_cycle(program_id)` → creates training record for the current player on a named program (program lookup resolves to activity_type + stat + skill + dc + mentor_id, parallel to HTTP `POST /api/activities` type=training). Paired with `query_training_programs()` so the DM can list available programs. Current DM verbs: `begin_activity` to start, `query_info` to list. <!-- evidence: apps/agent/training_tools.py initiate_training_cycle + query_training_programs; sprint-009 story-002 reworded the signature from (character_id, activity_type, target) per execution_plan.json §M-2 deep-dive (program_id is HTTP parity) -->
- Former agent tool: `resolve_training_midpoint(training_id, decision)` → advances to second half Current DM verb: `resolve_activity`. <!-- evidence: apps/agent/training_tools.py resolve_training_midpoint; sprint-009 story-003 shipped the tool, story-004 registered in CityAgent.CITY_TOOLS. Resource-row template (validate-id → for-update → ownership → state → rules → update). -->
...
- [x] Skill practice training increments the same use counter as session use (M1.2 hybrid advancement) <!-- evidence: apps/agent/async_worker_training.py:apply_skill_practice_advancement (Wave 2.5 extraction); pinned by apps/agent/tests/test_hybrid_counter.py -->
...
- [x] Tests cover full cycle for each activity type, including midpoint decision branches <!-- evidence: apps/agent/tests/test_training_integration.py, test_training_rules.py, worker_suite/ -->
...
**M1.5 Verification (sprint-009, paths updated):** All 7 acceptance criteria are green. The full-cycle integration case is `TestFullCycleViaFunctionTools` in `apps/agent/tests/test_training_integration.py`; split training tool coverage lives in `test_training_initiate.py`, `test_training_resolve.py`, and `test_training_agent.py`. Verified-at: `ba954a1`. The current DM verbs are `begin_activity` and `resolve_activity`; lookup uses `query_info`.
...
- Former agent tool: `dispatch_companion_errand(companion_id, errand_type, destination)` → creates errand record Current DM verb: `begin_activity`. <!-- see audit/phase-1-async.md#m1.6 — no @function_tool; dispatch is HTTP-only (POST /api/activities type=companion_errand). DM agent cannot dispatch an errand directly -->
- Former agent tool: `resolve_companion_errand(errand_id)` → generates return narration Current DM verb: `resolve_activity`. <!-- see audit/phase-1-async.md#m1.6 — resolve_companion_errand exists as a pure function in async_rules.py:203; the current DM entrypoint is `resolve_activity` -->
...
- [x] Return narration is pre-rendered and stored for Catch-Up feed delivery <!-- evidence: the async worker uses apps/agent/async_rules.py:203 to resolve errands and persists narration_text/summary/audio_url; catchup.ts:190 activityToFeedItem -->
- [x] Concurrency enforced: 3 independent slots (Training + Crafting + Companion). Artificer can craft in Training slot <!-- verified 2026-09-01: DEFERRAL DISCHARGED — all three of ADR 0005's Phase-5 requirements are met. (1) Portable Lab exists as item + recipe (content/items.json `artificers_portable_lab`, content/recipes.json:1427). (2) countActiveBySlot now buckets on `COALESCE(data->>'slot', data->>'activity_type')` (apps/server/src/activity_create.ts:19), closing debt 95de7fa141df. (3) The crafting create path loads class + lab ownership and passes them to validateSlotAvailability, stamping data.slot='training' on the borrow (activity_create.ts:184,267,334); Python mirror at apps/agent/crafting_tools.py:73 _resolve_crafting_slot. RED-capable tests: apps/server/src/slot_validation.test.ts:75-105 + apps/server/src/activity_create.test.ts:261,284,295 (40 pass, 0 fail); real-DB E2E apps/agent/tests/acceptance/test_artificer_slot_e2e.py asserts the borrow is COUNTED (follow-on training POST rejected) -->
...
**M1.6 Verification (sprint-010, updated after Phase 5):** 7 of 7 acceptance boxes green. The errand seam was verified by `apps/agent/tests/acceptance/test_m1_6_companion_errands.py`; the Artificer Training-slot exception shipped with Portable Lab and is verified by `apps/agent/tests/acceptance/test_artificer_slot_e2e.py`. ADR 0005 is fulfilled and debt `95de7fa141df` discharged. The current DM verbs are `begin_activity` and `resolve_activity`; lookup uses `query_info`.
