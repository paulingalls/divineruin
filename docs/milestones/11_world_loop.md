...
- **Rule-based fast path (no LLM):** the spec's simple cases — corruption rising in territory → ward strength up; unchallenged influence → maintain; aligned quest completed → small blessing. Encoded per-god as rules referencing Phase 8 domain definitions
