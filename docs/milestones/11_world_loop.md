...
- **Rule-based fast path (no LLM):** the spec's simple cases — corruption rising in territory → ward strength up; unchallenged influence → maintain; aligned quest completed → small blessing. Encoded per-god as rules referencing Phase 8 domain definitions. **→ M24 pointer:** the full-spec Veil Ward (SMM `veil-ward-scope-decision`) makes wards area-scoped and duration-bound; the **world-sim half — ambient territory Resonance, corruption-driven ward strength, seasonal escalation, and Sacred-site permanent world wards — rides Phase 11**, building on the M24 scope-model core. The "ward strength up" rule here is one such consumer.
