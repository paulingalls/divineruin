...
- [x] Workspace rental pricing matches the 4-row spec table with disposition discount tiers (Friendly 80%, Trusted 60%) <!-- verified 2026-09-01: apps/agent/workspace.py RENTAL_BASE_PRICE_SP (workshop 2 / forge 5 / laboratory 10) + COMBINED_FORGE_LAB_RENTAL_SP=12 + compute_rental_price() (workspace rentals go through compute_workspace_rental_price, which zeroes the trusted tier); discounts from the DB-loaded SSOT content/pricing.json disposition_multipliers {friendly 0.8, trusted 0.6} via pricing_queries.get_economy_pricing(); debited live in crafting_tools.rent_workspace. Tests apps/agent/tests/test_workspace.py::TestRentalPricing (4 base rows + neutral/friendly/trusted + below-neutral refusal), apps/server/src/pricing.test.ts -->
