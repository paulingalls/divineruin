...
- Former agent tool: `learn_recipe(character_id, recipe_id, acquisition_method)` → adds recipe to known list, validates slot capacity The current DM verb is `learn`.
- Former agent tool: `query_recipe_requirements(recipe_id)` → returns materials, workspace, DC, and time requirements The current DM verb is `query_info`.
...
- Former agent tool: `query_available_workspaces(character_id, location_id)` → list of accessible workspaces with costs The current DM verb is `query_info`.
- Former agent tool: `start_crafting_project(character_id, recipe_id, workspace_id)` → runs three-check pipeline, rolls if valid The current DM verb is `begin_activity`.
...
- [x] NPC rental costs match spec: Workshop 2sp, Forge 5sp, Lab 10sp, Combined 12sp per day (the Combined price was a constant nothing could charge until sprint-047 story-015 made it the `forge_laboratory` rental offer — quoted by `query_info(kind="workspaces")`, charged by `begin_activity(kind="workspace")`, and persisted as TWO `workspace_rentals` rows so the four-member TS vocabulary in `apps/server/src/workspace.ts` stays closed. `test_crafting_tools_bundle.py::TestQuoteMatchesCharge::test_every_quoted_token_is_rentable` is the guard)
...
- [x] Trusted reputation grants free workspace access at that settlement (shipped sprint-045 story-005 as the trusted DISPOSITION tier: the free branch is at `compute_workspace_rental_price`; shared `compute_rental_price` applies the multiplier and rounds the quote half up to whole silver for repairs and rentals. The `forge_laboratory` bundle is offered only where location tags host both grants, by BOTH rental and quote; settlement gating of single rentals remains deferred by note `2301b33e`)
...
- [x] `start_crafting_project` fails gracefully with clear reason when any check fails The current DM verb is `begin_activity`.
...
**Resolved:** Trusted free access and workspace co-location validation shipped; the retired `rent_workspace` behavior is reached through `begin_activity`.
...
- **Economy milestone:** workspace rental through `begin_activity` still uses interim 10sp=1gp fractional gold; reconcile when currency sub-units are formalized (concern `67c8f2962302`).
...
- Former agent tool: `experiment_with_materials(character_id, materials, intended_output)` → experimentation result The current DM verb is `begin_activity`.
...
- **Economy milestone:** workspace rental fractional-gold reconciliation in `begin_activity` (concern `67c8f2962302`).
...
- **Phase-6 / economy:** fractional-gold reconciliation (`67c8f2962302`), future single-rental settlement gating (note `2301b33e`).
