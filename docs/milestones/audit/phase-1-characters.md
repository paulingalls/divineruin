...
- Spec deliverable named `calculate_level(total_xp) -> level`; sprint-008 shipped `level_for_xp(total_xp) -> int` at `apps/agent/rules_engine.py:274` as the canonical standalone. `check_level_up` now delegates via `max(current_level, level_for_xp(new_xp))`. The original spec name differs but the standalone XP→level function exists; acceptance criteria do not require the spec name.
...
| Specialization fork at L5 is flagged in level-up rewards (L4 emits `elective_techniques`) | `rules_engine.py:261` `SPECIALIZATION_LEVEL = 5`; `leveling.py:85-92` L5 entry has `specialization_fork=True, milestone_type="specialization"`. L4 carries `milestone_type="elective_techniques"` and `attribute_points=2` but `specialization_fork=False`. | `test_rules_leveling.py:113-135` (fork at L5, not L4, multi-level includes, past-fork no-flag); `test_leveling.py:39` `test_specialization_fork_only_at_l5` (explicitly asserts L4 is NOT a fork) | confirmed (post-sprint-008 reword) — settled by sprint-008 in `01_core_systems.md §M1.4`: spec reworded to L5-only fork per `game_mechanics_core.md L656` "L5 = identity"; L4 remains `elective_techniques`. |
...
Sprint-008 struck the `apply_level_up(character_id)` named-tool deliverable from `01_core_systems.md §M1.4` and accepted the indirect path: level-up is emitted as a side effect of `award_xp` (and `complete_quest`), which calls `check_level_up` and publishes `LEVEL_UP` with the full reward payload.

| Spec name | Actual | File:line | Status |
| --- | --- | --- | --- |
| `apply_level_up(character_id)` agent tool | Wrapped inside `award_xp` agent tool — calls `check_level_up`, persists XP/level via `update_player_xp`, publishes `LEVEL_UP` event with full reward payload | `apps/agent/progression_tools.py:26` (tool); `:67` (check_level_up); `:69` (update_player_xp); `:88` (LEVEL_UP publish) | struck (sprint-008) — deliverable removed; indirect path accepted |

The level-up acceptance criteria (narration event, attribute points, specialization fork) are all met via this indirect path. Sprint-008 named the rationale at `01_core_systems.md §M1.4` deliverable L125 ("second entrypoint would introduce drift risk") and recorded the strike in the M1.4 Verification footer at L142.
...
3. **`level_for_xp` standalone shipped.** Sprint-008 added `level_for_xp(total_xp) -> int` at `apps/agent/rules_engine.py:274` per `game_mechanics_core.md L678` reference implementation. `check_level_up` was consolidated to delegate via `max(current_level, level_for_xp(new_xp))`. The original spec name (`calculate_level`) differs from the shipped name but the standalone function exists; no blocked acceptance criterion.
