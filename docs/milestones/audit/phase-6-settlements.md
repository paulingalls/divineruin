...
| Bandit Ambush (npcs.md:601-606) | M6.2 — hostile encounter templates | Composition (3-5 Bandits + 1 Bandit Captain), tactics, morale, loot all spec'd. NEW vs milestone acceptance — milestone §Deliverables lists "Hostile encounter templates using companions: Bandit Ambush, Ashmark Patrol, Cult Cell, Hollow-Corrupted Settlement" but the 8 acceptance bullets do NOT mention them. **Same 4 templates also enumerated in M6.4 §Deliverables L130** — sprint-005 capstone (story-005) must record a single canonical owner (recommend M6.2, since M6.2 owns the settlement context the encounters fire in; M6.4 references them only as companion-using). |
...
# Expected hits: 2 narrative descriptors of god Veythar in content/voice_registry.json + content/gods.json
# ("Scholarly warmth" — not a settlement-trait binding). Any other hits = audit went stale.
grep -rnE '\bProsperous\b|\bStruggling\b|\bScholarly\b|\bDevout\b|\bFrontier\b|\bRefuge\b' apps/ packages/ content/
