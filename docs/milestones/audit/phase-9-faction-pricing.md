...
**Honesty note 3 — Spec worked examples (Thornwatch + Merchant Guild) don't match shipped content.** Spec L115-129 provides two faction worked examples: Thornwatch (military) and Merchant Guild (commercial). Neither appears in `content/factions.json`. Spec L66 also references "Thornwatch outpost". The 4 shipped factions (`accord_guild`, `aelindran_diaspora`, `independent`, `temple_authority`) are an **adjacent but non-overlapping set** — the Accord Guild is the closest analogue to the Merchant Guild example; no Thornwatch-equivalent exists. `temple_authority` ships but spec uses only "temple" as a generic noun (L48 "donate to faction temple or cause"), not as a worked-example faction. **Content-spec drift**: spec examples reference factions not in content; content factions are not covered by spec examples. Capstone should reconcile (either backfill Thornwatch in content, or update spec to use shipped faction ids).
