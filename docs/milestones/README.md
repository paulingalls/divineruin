...
| 2 · Archetypes | [02_archetypes.md](02_archetypes.md) | ✅ **Complete** | 34/34 ACs | The three residuals this row carried are all closed and the doc's boxes match: reactions declare and gate (`declarations.py:42` REACTION + `combat_phase.validate_reaction_activation`, sprint-045; windows surfaced by `query_info(kind="abilities")`, sprint-046 story-007); variant supplement closed by M9 story-004 (`activate` routes variant ids as a fourth namespace); L20 legendary companion closed sprint-046 story-009 (the assigned companion's L20 gain rides the XP Resolve). |
