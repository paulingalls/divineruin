...
Audio duration is the sum of frames sent. The latency boundary ends when the DM's LLM generation starts; it does not include LLM or TTS time to first audio. The uncontended 0.774001 s leading measurement is below the architecture's 1.2–2.0 s speech-end-to-first-audio target, but it cannot prove that end-to-end target because no audio was generated. A queued turn intentionally includes time spent waiting for the preceding reply and tool chain; this trace released the gate immediately after verifying the wait. This is one diagnostic run, not a percentile or latency guarantee.
