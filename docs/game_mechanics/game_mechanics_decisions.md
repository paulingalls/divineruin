...
## Terrain (Decisions 129-132)

129. **Terrain is a single closed enum authored on the `Location` entity; every terrain-gated rule derives from it rather than declaring its own terrain field.** Reason: terrain was authored as content and referenced across magic, archetypes, patrons, bestiary, and items (81 references in the specs) but never defined as a system — there was no design decision governing it and no runtime field saying "this location's terrain is X." The result was four incompatible vocabularies (Resonance-yield bands, "earth or stone," biome names, tactical "difficult/narrow") and a family of rules shipped with the terrain condition silently deferred. One authored field with deterministic derivations is chosen over parallel terrain fields because parallel fields drift, and drift is exactly the failure mode already observed (a Druid able to raise a natural-terrain ward in a city, a Korath getting the earth-stone reduction on any terrain). The enum is closed and strict-loader-validated, failing loud on unknown values — consistent with the project's existing `resonance.py` precedent. Tactical terrain ("difficult," "narrow") is explicitly carved out: it is a transient, position-level combat feature flagged per-encounter, not a property of a location, and never enters the Location terrain enum.

130. **The canonical terrain enum has ten static values, and the four legacy vocabularies are reconciled into deterministic per-value properties.** Reason: a single source of truth is required before any runtime work, or the wrong thing gets built. Each enum value carries three derived properties — a base primal-Resonance multiplier, an `is_natural` flag (Druid Veil Ward, Thyra patron bonuses), and an `is_earth_or_stone` flag (Korath reduction). "Natural terrain," "earth or stone," and the Resonance band stop being separate vocabularies and become lookups from the one enum.

    | terrain | example regions | resonance_base | is_natural | is_earth_or_stone |
    |---|---|---|---|---|
    | `ancient_forest` | deep Thornveld, sacred groves | 0.1 | yes | yes |
    | `forest` | Thornveld woodland | 0.2 | yes | yes |
    | `grassland` | Drathian Steppe, plains | 0.2 | yes | yes |
    | `mountain` | Keldaran Holds | 0.2 | yes | yes |
    | `coast` | Sunward Coast shoreline | 0.2 | yes | no |
    | `wetland` | Sunward Wetlands, marsh | 0.2 | yes | no |
    | `underground` | Umbral Deep, caves | 0.2 | yes | yes |
    | `farmland` | settled croplands | 0.3 | yes | yes |
    | `village` | small towns, outposts | 0.4 | no | no |
    | `city` | Sunward Accord cities | 0.5 | no | no |

    Derivations: Druid Veil Ward (`game_mechanics_magic.md:207`) is allowed iff `is_natural`, with the "stronger in old-growth / sacred groves" clause applying only to `ancient_forest`; Korath -1 primal (`game_mechanics_magic.md:256`) applies iff `is_earth_or_stone`, with `coast` and `wetland` deliberately excluded (sand/mud/water, not earth contact); Thyra natural-terrain bonuses (`game_mechanics_patrons.md:128,131`) apply iff `is_natural`; Warden "Rooted" home terrain (`game_mechanics_archetypes.md:727`) picks one of the five biome values `{forest, mountain, coast, grassland, underground}` (existing "plains" maps to `grassland`), and a Warden whose home is `forest` treats `ancient_forest` as home as well. Accepted abstraction: `underground` is `is_natural=yes`, so a built/stone dungeon that happens to be underground reads as natural terrain for the ward — a known simplification chosen over enum proliferation (a `natural` vs `built` underground split), in the same spirit as the half-CON floor-division dead zone: simpler and conservative, low stakes given the ward is a level-9 ability.

    **Amendment (region_type reconciliation):** a coarse `region_type` enum `{city, wilderness, dungeon}` already exists on `Location` and is load-bearing for agent tool scoping, so terrain **refines** it rather than replacing it: `dungeon`→`underground`; `city`→`{city, village}`; `wilderness`→ the seven nature values (`ancient_forest, forest, grassland, mountain, coast, wetland, farmland`). The strict loader rejects any terrain inconsistent with its entry's `region_type`. This relationship was surfaced during milestone scoping (`10_terrain.md` M10.1); Decision 130 was originally written as if `Location` had no terrain axis.

131. **The primal Resonance multiplier is computed as `resonance_base[terrain]` adjusted by a corruption level, replacing the standalone `PRIMAL_TERRAIN_TABLE` as an independent vocabulary.** Reason: the old eight-band table mashed together two axes — base biome (forest/natural/farmland/village/city) and corruption (damaged/hollow-adjacent/hollow-corrupted). The bottom five bands are now the `resonance_base` values in Decision 130; the top three become corruption levels on the deferred dynamic world-state axis. Corruption currently defaults to 0 (none), so primal Resonance is fully functional today on static terrain and the future dynamic-corruption system slots in by raising the corruption level — no re-architecture. This un-deads a signature pillar: until now, the primal terrain routing path was dead (catalog baselines bypassed it), which meant geographically-dependent primal magic was inert — every primal cast generated the same Resonance regardless of location, while the DM was scripted to narrate "the stone absorbs it" with no mechanical backing. The world now actually feels different to cast in. The stale `_DEFAULT_TERRAIN = "normal"` default is removed: `"normal"` was never a key in the table (a latent KeyError masked only by the dead path), and a closed enum plus mandatory content backfill means there is nothing to default to.

132. **Terrain resolves once at combat start and is cached as a derived field on `CombatState`; world and exploration systems read `Location.terrain` directly.** Reason: terrain is authored on `Location` (the single source of truth) and is never authored on `CombatState` — caching it as a derived value at combat start avoids the drift risk of a second authored field while avoiding a per-cast location lookup. Per-cast Resonance resolution reads the cached value. This respects the audio-first 1500 ms latency budget — no database round-trip mid-combat — and matches the voice-first principle that a player should never have to track or be asked about terrain state; the engine resolves it silently and the DM narrates it.
