...
One consumer sends a complete turn to the DM and awaits its full reply and tool chain before taking the next turn. Near-simultaneous speakers therefore receive separate ordered responses, and every tool in a response runs with only that turn's authenticated actor bound. Actor-aware resource verbs resolve that binding and revalidate its connection generation at the write boundary. A primary-only verb that has not adopted actor ownership fails before it can read the primary player id during another player's turn, rather than falling back to the primary player's resources. Input sessions use a 1.0 s minimum endpointing delay so a natural pause does not split one utterance. Once endpointing completes, a solo utterance enters the consumer immediately; there is no multi-speaker collection window.
...
3. **DM Agent — basic voice loop.** The original `DungeonMasterAgent` proved the voice loop; `ExplorationAgent` now handles gameplay conversation on GPT-6 Luna with the DM persona.
