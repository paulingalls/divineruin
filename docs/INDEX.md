...
| Section | Lines | What's There |
|---|---|---|
| Why this exists | 11-32 | The limits: tool ceiling, duplicated consequence logic, content-driven tool growth |
| Core model: Sense / Act / Resolve | 33-51 | The three operation kinds; Resolve is never a tool |
| Sense channels: push + pull | 52-72 | The Stage (push) vs `query` (pull); the cost/latency line; three ceiling levers |
| Acts are verbs, never nouns | 73-111 | Decision test, verb test, description discipline, standard Act shape + ActResult |
| Resolve is never a tool | 112-135 | Fires inside Acts; resolve_milestone removed (grant=Resolve, choice=select); report back richly |
| Agents vs Stages | 136-179 | Two axes, handoff/cache cost; collapse region agents into one exploration agent |
| The Stage in detail | 180-315 | Cold/warm/hot tiers, placement discriminator, numbers→bands, check(skill,target), stage schema |
| The loop and three actors | 316-339 | Stage→Decide→Act→Resolve→Stage; user/LLM/world; coherence guardrail; event bus |
| Worked example: Guild Hall | 340-369 | Real content rendered through the schema |
| Verb vocabulary | 370-425 | 12 core verbs + mode-local sets; what each replaces; what folded |
| Migration sequence | 426-444 | Five green-throughout steps; M2.4 adds zero tools |
| Decisions and deferrals | 445-486 | Resolved seams + deferrals with insertion points |
| Relationship to Golden Rules | 487-499 | How the model maps to the seven rules |

---

## agent_tool_surface.md (605 lines)
...
| The limits, precisely | 96-134 | 20 strict tools, 16 unions (recursive), ~13-nullable object cliff, additionalProperties/enum-null/oneOf rejections |
| Diagnosis | 135-156 | Why folding nouns into verbs produced product-types-with-nulls |
| Verbs take sum types | 159-247 | The schema rules + per-verb rewrites (check, begin_activity, declare_phase, activate, travel, enter_mode) |
| When a verb set grows | 248-265 | Split criteria: eval-driven, not count-driven |
| Right information at the right time | 266-309 | Layer-by-volatility placement rule; the ACTIVE COMBAT staleness bug |
| Right action at the right time | 310-371 | Why per-turn tool scoping thrashes the cache; the NOW block + `next` field |
| Plugin constraints and escape hatches | 372-392 | raw_schema, non-strict mixing, what LiveKit emits |
| Sequencing | 393-430 | Order of work for story-019 (Sprint 47, ahead of the M29 restore) |
| Risks and how to measure them | 431-452 | Tool-selection eval, cache-read assertion |
| Open questions for the human | 453-467 | query_info shape, enter_mode, resolve_phase-as-advance-verb, creation agent model |
| Appendix — how this was measured | 468-490 | The schema walk and the live limit probes |
| Cost recommendations | 491-605 | Cache protection, fewer turns/session, models, TTS, a cost budget test |
